...
function fillTheTable() {
    ...
    for (let i = 1; i <= length; ++i) {
        ...
        console.log(item);
        tableText += "<tr id='tr" + i + "'>" + "<td style='width: 5%'>" + i + "</td>" + "<td>" + "<img style='width: 200px;' src=\"resources/" + item.image + "\" alt =\"" + item.name + "\" width =\"300\">" + "</td>" + "<td style='width: 50%'>" + "<a href='item.html?id=" + i + "&type=" + item.type + "&title=" + item.title + "&description=" + item.description + "&value=" + item.value + "&image=" + item.image + "'>" + item.title + "" + "</td>" + "<td>" + item.value + "</td>" + "<td>" + "<button class=\"deleteBtn\" onclick='deleteItemFromCart(keys[" + i + "-1], " + i + ")'>Delete</button>" + "</td>" + "</tr>";
    }
